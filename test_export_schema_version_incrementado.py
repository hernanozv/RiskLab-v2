...
El fix incrementa EXPORT_SCHEMA_VERSION a "1.1" y actualiza las 3
referencias a la versión en EXPORT_SCHEMA.md (encabezado + 2 ejemplos
JSON) para que queden consistentes con el código.

Este test verifica que:
1. EXPORT_SCHEMA_VERSION en el código sea "1.1", no "1.0".
2. El payload real construido por _construir_export_payload_ia refleje
   ese mismo valor en "$schema_version".
3. EXPORT_SCHEMA.md ya no mencione la versión vieja "1.0" en ningún
   lugar donde documenta la versión del schema.
"""
import os
import sys
...
check(RLB.EXPORT_SCHEMA_VERSION == "1.1",
      f"Bug bajo R4 #1: EXPORT_SCHEMA_VERSION es '1.1', no la vieja '1.0' "
...
check('`1.1`' in schema_md,
      "EXPORT_SCHEMA.md documenta la versión '1.1' en su encabezado")
check('"$schema_version": "1.0"' not in schema_md,
      "Bug bajo R4 #1: EXPORT_SCHEMA.md ya NO menciona la versión vieja '1.0' "
      "en sus ejemplos JSON")
check('`1.0`' not in schema_md,
      "Bug bajo R4 #1: EXPORT_SCHEMA.md ya NO menciona '1.0' como versión del schema")
